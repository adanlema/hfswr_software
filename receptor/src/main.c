            ...
            switch (addr_ctrl_regs[BUFFER_TO_READ]) {
                case 1:
                    ...
                    for (addrs_t p = addr_buff_1; p < (addr_buff_1 + 200); ++p) {
                        // for (addrs_t p = addr_buff_1; p < (addr_buff_1 +
                        // (addr_ctrl_regs[LAST_ADDR] == 0xFFFFFFFF)? 4095 :
                        // addr_ctrl_regs[LAST_ADDR]); ++p) {
                        sprintf(s_buff, "\n%d", (int16_t)(0xFFFF & (*p)));
                        send(confd, s_buff, BUFTCP_SIZE, 0);
                    }
                ...
                case 2:
                    ...
                    sprintf(s_buff, "\nDatos buffer dos:");
                    send(confd, s_buff, BUFTCP_SIZE, 0);
                    // for (addrs_t p = addr_buff_2; p < (addr_buff_2 + (addr_ctrl_regs[LAST_ADDR]
                    // == 0xFFFFFFFF)? 4095 : addr_ctrl_regs[LAST_ADDR]); ++p) {
...
int main() {
    ...
    // Valores iniciales
    // addr_ctrl_regs[PHASE_CARRIER] = 0x14d5555c; // Frecuencia de portadora en 10 MHz - No deberia
    // tocar esto desde aqui!!!
    addr_ctrl_regs[START_FPGA] = 0;
    addr_ctrl_regs[ADDR_RESET] = 1;
    addr_ctrl_regs[WRITE_EN]   = 3;
    addr_ctrl_regs[START_FPGA] = 1;
